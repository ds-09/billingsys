...
class PDF(FPDF): 
    def header(self): 
        ...
        self.cell(0, 5, 'GSTIN: 24AAJCB3255xxxx | Phone: 9974040755', 0, 1, 'C') 
...
def create_invoice_pdf(invoice_data): 
    ...
    for index, row in items_df.iterrows(): 
        ...
        # Print single-line content, vertically centered
        y_centered = y_start + (row_height - line_height) / 2  # Updated calculation for vertical centering

        try:
            hsn_code = str(int(row['HSN CODE'])) if pd.notna(row['HSN CODE']) else ''
        except (ValueError, TypeError):
            hsn_code = str(row['HSN CODE'])

        pdf.set_xy(x_start + sum(col_widths[:3]), y_centered)
        pdf.cell(col_widths[3], 0, hsn_code, 0, 0, 'C')

        pdf.set_xy(x_start + sum(col_widths[:4]), y_centered)
        pdf.cell(col_widths[4], 0, str(row['QTY']), 0, 0, 'R')

        pdf.set_xy(x_start + sum(col_widths[:5]), y_centered)
        pdf.cell(col_widths[5], 0, f"{row['RATE']:.2f}", 0, 0, 'R')

        pdf.set_xy(x_start + sum(col_widths[:6]), y_centered)
        pdf.cell(col_widths[6], 0, f"{row['AMOUNT']:.2f}", 0, 0, 'R')
...
if submitted: 
    ...
    if final_df.empty or final_df['QTY'].sum() == 0: 
        ...
        invoice_data = { 
            ...
            "remarks": remarks, "bank_name": "BANK", "bank_branch": "Road", "bank_ac_no": "50000000009999", "bank_ifsc": "H000009999" 
        } 
